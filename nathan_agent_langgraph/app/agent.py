...
def generate_Nathans_calendar() -> dict[str, list[str]]:
    """Generates Nathan's calendar for the next 7 days."""
    calendar = {}
    today = date.today()
    # Nathan's availability: evenings on weekdays, more free on weekends.
    for i in range(7):
...
NathanS_CALENDAR = generate_Nathans_calendar()
...
def get_availability(date_range: str) -> str:
    """Use this to get Nathan's availability for a given date or date range."""
    ...
    try:
        ...
        for i in range(delta.days + 1):
            ...
            available_slots = NathanS_CALENDAR.get(date_str, [])
            if available_slots:
                availability = (
                    f"On {date_str}, Nathan is available at: "
                    f"{', '.join(available_slots)}."
                )
                results.append(availability)
            else:
                results.append(f"Nathan is not available on {date_str}.")
...
class NathanAgent:
    """NathanAgent - a specialized assistant for scheduling."""

    SUPPORTED_CONTENT_TYPES = ["text", "text/plain"]

    SYSTEM_INSTRUCTION = (
        "You are Nathan's scheduling assistant. "
        "Your sole purpose is to use the 'get_availability' tool to answer questions about Nathan's schedule for playing badminton. "
        ...
        "If the user asks about anything other than scheduling badminton, "
        ...
        for item in self.graph.stream(inputs, config, stream_mode="values"):
            ...
            if (
                ...
                yield {
                    ...
                    "content": "Checking Nathan's availability...",
                }
