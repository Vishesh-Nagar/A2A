...
def generate_john_calendar() -> dict[str, list[str]]:
    """Generates a random calendar for John for the next 7 days."""
    calendar = {}
    today = date.today()
    ...
    print("John's calendar:", calendar)

    return calendar


JOHN_CALENDAR = generate_john_calendar()


def get_availability(start_date: str, end_date: str) -> str:
    """
    Checks John's availability for a given date range.
    ...
    Returns:
        A string listing John's available times for that date range.
    """
    try:
        ...
        for i in range(delta.days + 1):
            ...
            available_slots = JOHN_CALENDAR.get(date_str, [])
            if available_slots:
                availability = f"On {date_str}, John is available at: {', '.join(available_slots)}."
                results.append(availability)
            else:
                results.append(f"John is not available on {date_str}.")
...
def create_agent() -> LlmAgent:
    """Constructs the ADK agent for John."""
    return LlmAgent(
        model="gemini-2.5-flash",
        name="John_Agent",
        instruction="""
            **Role:** You are John's personal scheduling assistant. 
            Your sole responsibility is to manage his calendar and respond to inquiries 
            about his availability for badminton.

            **Core Directives:**

            *   **Check Availability:** Use the `get_john_availability` tool to determine 
                    if John is free on a requested date or over a range of dates. 
